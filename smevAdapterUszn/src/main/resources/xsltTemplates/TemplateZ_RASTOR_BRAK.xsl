...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:call-template name="root">
            <xsl:with-param
                    name="title"
                    select='"Сведения об актах гражданского состояния о расторжении брака, содержащихся в Едином государственном реестре записей актов гражданского состояния"'/>
        </xsl:call-template>
    </xsl:template>
        ...
        <table class="table2">
            <tr class="tr2">
                <td class="td2" width="400">
                    Фамилия:
                    <xsl:value-of select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/ФИО/Фамилия"/>
                    <br/>
                    Имя:
                    <xsl:value-of select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/ФИО/Имя"/>
                    <br/>
                    <xsl:if test="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/ФИО/Отчество!=''">
                        Отчество:
                        <xsl:value-of select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/ФИО/Отчество"/>
                    </xsl:if>
                    <br/>
                    Дата рождения:
                    <xsl:call-template name="dateMy">
                        <xsl:with-param name="value"
                                        select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/ДатаРождКаленд"/>
                    </xsl:call-template>
                    <br/>

                    <xsl:if test="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/@СНИЛС != ''">
                        СНИЛС:
                        <xsl:value-of select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведФЛ/@СНИЛС"/>
                        <br/>
                        Номер записи:
                        <xsl:value-of select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведАГС/@НомерЗапис"/>
                        <br/>
                        Дата записи:
                        <xsl:call-template name="dateMy">
                            <xsl:with-param name="value"
                                            select="MessagePrimaryContent/BRAKRASTINFRequest/СведЗапрос/СведАГС/ДатаЗапис"/>
                        </xsl:call-template>
                    </xsl:if>
                    <br/>
                </td>
                    ...
                    <xsl:variable name="кодОсн">
                        <xsl:call-template name="кодОСН">
                            <xsl:with-param name="kod"
                                            select="MessagePrimaryContent/BRAKRASTINFRequest/СведОсн/@КодНормОсн"/>
                        </xsl:call-template>
                    </xsl:variable>
    ...
    <xsl:template match="reportResponse">
        ...
        <table class="table1">
            <xsl:call-template name="tableInsertRow">
                ...
                <xsl:with-param name="col2" select="BRAKRASTINFResponse/@ИдЗапрос"/>
            </xsl:call-template>

            <xsl:variable name="ТипАГСЗапрос">
                <xsl:call-template name="ZagsТипАГСЗапрос">
                    <xsl:with-param name="kod" select="BRAKRASTINFResponse/@ТипАГСЗапрос"/>
                </xsl:call-template>
            </xsl:variable>
            <xsl:call-template name="tableInsertRow">
            ...
            <xsl:variable name="РезОбраб">
                <xsl:call-template name="ZagsРезОбраб">
                    <xsl:with-param name="kod" select="BRAKRASTINFResponse/СведОтветАГС/@РезОбраб"/>
                </xsl:call-template>
            </xsl:variable>
            <xsl:call-template name="tableInsertRow">
            ...
            <xsl:apply-templates select="BRAKRASTINFResponse"/>
        </table>
    </xsl:template>
    ...
    <!--СведРегРастБрак-->
    <xsl:template match="СведРегРастБрак">
        ...
        <xsl:apply-templates select="СвидетРастБрак" mode="расторжениеБрака"/>


        <xsl:call-template name="tableInsertRow">
            <xsl:with-param name="col1"
                            select="'Передаваемые сведения о государственной регистрации расторжения брака'"/>
    ...
    <xsl:template match="СвидетРастБрак" mode="расторжениеБрака">
        <xsl:call-template name="СведенияОСвидельстве">
