...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        <xsl:call-template name="root">
            <xsl:with-param
                    name="title"
                    select='"Сведения о регистрации по месту пребывания граждан РФ"'/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="reportRequest">
       <b>Сведения физического лица: </b> <br/>
        Фамилия: <xsl:value-of select="MessagePrimaryContent/stayingPlaceRegistrationRequest/personInfo/lastName"/>
       <br/>
        Имя: <xsl:value-of select="MessagePrimaryContent/stayingPlaceRegistrationRequest/personInfo/firstName"/>
        <br/>
        <xsl:if test="MessagePrimaryContent/stayingPlaceRegistrationRequest/personInfo/middleName!=''">
            Отчество: <xsl:value-of select="MessagePrimaryContent/stayingPlaceRegistrationRequest/personInfo/middleName"/>
            <br/>
        </xsl:if>
        Дата рождения:
        <xsl:call-template name="dateMy">
            <xsl:with-param name="value" select="MessagePrimaryContent/stayingPlaceRegistrationRequest/personInfo/birthDate"/>
        </xsl:call-template>
        <br/>
        Тип документа:
        <xsl:call-template name="TYPE_DOC">
            <xsl:with-param
                    name="t"
                    select="MessagePrimaryContent/stayingPlaceRegistrationRequest/document"/>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="reportResponse">
        ...
        <br/>
        <xsl:if test="stayingPlaceRegistrationResponse/notFoundRegistration">
            <b>Статус обработки запроса:</b> Информация о регистрации не найдена
            <br/>
        </xsl:if>
       <xsl:apply-templates select="stayingPlaceRegistrationResponse/registrationInfo"/>
    </xsl:template>
        ...
        <table class="table1">
            <xsl:call-template name="tableInsertRow">
                <xsl:with-param name="col1"
                                select="'Описание ответа на запрос о регистрации по месту пребывания граждан РФ '"/>
