...
import smev.ElementServices.ZagsRename.PERNAMEINFRequest;
import smev.ElementServices.ZagsRename.PERNAMEINFResponse;
import smev.ElementServices.ZagsRename.ФИОПрТип;
...
@Component
public class PERNAMEINFRequestRequest extends ObjectMessage implements TypeBussines, CreateMessageStrategy, ReceiveMessageStrategy {

    private PERNAMEINFRequest data;
    ...
    public ObjectMessage runOrder(BusinessMessageEntity messageEntity, UUID inquiryVersionId, PayLoad payLoad) throws ParseException, DatatypeConfigurationException {
        ...
        ObjectMessage message = new PERNAMEINFRequestRequest();
        PERNAMEINFRequest dataRequest = new PERNAMEINFRequest();
        SubjectDataDTO subjectDTO = (SubjectDataDTO) payLoad.getPayLoadObject();

        dataRequest.setКолДок(BigInteger.valueOf(1));
        dataRequest.setТипАГС("06");

        PERNAMEINFRequest.СведЗапрос.СведФЛ сведФЛ = new PERNAMEINFRequest.СведЗапрос.СведФЛ();
...
//        органЗАГСНТип.setНаимЗАГС();
        ...
        PERNAMEINFRequest.СведЗапрос сведЗапрос = new PERNAMEINFRequest.СведЗапрос();
...
//        сведЗапрос.setСведАГС(сведАГС);
        ...
        PERNAMEINFRequest.СведОсн сведОсн = new PERNAMEINFRequest.СведОсн();
        ...
        ((PERNAMEINFRequestRequest) message).data = dataRequest;
    ...
    public DataSubjectEntity runReceiveOrder(BusinessInteractionEntity interactionEntity, OrderInquiryDto orderInquiryDTO) throws ParseException, DatatypeConfigurationException {
        ...
        if (interactionInfoDto.getResponseNumber() > 0) {
            ...
            PERNAMEINFResponse response = (PERNAMEINFResponse) Marshaller.unmarshal(element);
    ...
    public String getIdInquery() {
        return InquiryVersionEnum.getInquiryId(InquiryVersionEnum.ZAGS_RENAME).toString();
    }
}
