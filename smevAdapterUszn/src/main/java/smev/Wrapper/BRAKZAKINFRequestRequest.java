...
import smev.ElementServices.ZagsBrakZak.BRAKZAKINFRequest;
import smev.ElementServices.ZagsBrakZak.BRAKZAKINFResponse;
import smev.ElementServices.ZagsBrakZak.ФИОПрТип;
...
@Component
public class BRAKZAKINFRequestRequest extends ObjectMessage implements TypeBussines, CreateMessageStrategy, ReceiveMessageStrategy {

    private BRAKZAKINFRequest data;
    ...
    @Override
    public BRAKZAKINFRequest tt(BusinessServiceImpl businessService, BusinessMessageEntity messageEntity) {
        return this.data;
    }
    ...
    public ObjectMessage runOrder(BusinessMessageEntity messageEntity, UUID inquiryVersionId, PayLoad payLoad) throws ParseException, DatatypeConfigurationException {
        ...
        ObjectMessage message = new BRAKZAKINFRequestRequest();
        BRAKZAKINFRequest dataRequest = new BRAKZAKINFRequest();
        SubjectDataDTO subjectDTO = (SubjectDataDTO) payLoad.getPayLoadObject();

        dataRequest.setКолДок(BigInteger.valueOf(1));
        dataRequest.setТипАГС("02");

        BRAKZAKINFRequest.СведЗапрос.СведФЛ сведФЛ = new BRAKZAKINFRequest.СведЗапрос.СведФЛ();
...
//        органЗАГСНТип.setНаимЗАГС();
        ...
        BRAKZAKINFRequest.СведЗапрос сведЗапрос = new BRAKZAKINFRequest.СведЗапрос();
...
//        сведЗапрос.setСведАГС(сведАГС);
        ...
        BRAKZAKINFRequest.СведОсн сведОсн = new BRAKZAKINFRequest.СведОсн();
        ...
        ((BRAKZAKINFRequestRequest) message).data = dataRequest;
    ...
    public DataSubjectEntity runReceiveOrder(BusinessInteractionEntity interactionEntity, OrderInquiryDto orderInquiryDTO) throws ParseException, DatatypeConfigurationException {
        ...
        if (interactionInfoDto.getResponseNumber() > 0) {
            ...
            BRAKZAKINFResponse response = (BRAKZAKINFResponse) Marshaller.unmarshal(element);
    ...
    public String getIdInquery() {
        return InquiryVersionEnum.getInquiryId(InquiryVersionEnum.ZAGS_ZAK_BRAK).toString();
    }
}
