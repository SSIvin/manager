...
import smev.ElementServices.ZagsDeath.FATALINFRequest;
import smev.ElementServices.ZagsDeath.FATALINFResponse;
import smev.ElementServices.ZagsDeath.ФИОПрТип;
...
@Component
public class FATALINFRequestRequest extends ObjectMessage implements TypeBussines, CreateMessageStrategy, ReceiveMessageStrategy {

    private FATALINFRequest data;
    ...
    public ObjectMessage runOrder(BusinessMessageEntity messageEntity, UUID inquiryVersionId, PayLoad payLoad) throws ParseException, DatatypeConfigurationException {
        ...
        ObjectMessage message = new FATALINFRequestRequest();
        FATALINFRequest dataRequest = new FATALINFRequest();
        SubjectDataDTO subjectDTO = (SubjectDataDTO) payLoad.getPayLoadObject();

        dataRequest.setКолДок(BigInteger.valueOf(1));
        dataRequest.setТипАГС("07");

        FATALINFRequest.СведЗапрос.СведФЛ сведФЛ = new FATALINFRequest.СведЗапрос.СведФЛ();
...
//        органЗАГСНТип.setНаимЗАГС();
        ...
        FATALINFRequest.СведЗапрос сведЗапрос = new FATALINFRequest.СведЗапрос();
...
//        сведЗапрос.setСведАГС(сведАГС);
        ...
        FATALINFRequest.СведОсн сведОсн = new FATALINFRequest.СведОсн();
        ...
        ((FATALINFRequestRequest) message).data = dataRequest;
    ...
    public DataSubjectEntity runReceiveOrder(BusinessInteractionEntity interactionEntity, OrderInquiryDto orderInquiryDTO) throws ParseException, DatatypeConfigurationException {
        ...
        if (interactionInfoDto.getResponseNumber() > 0) {
            ...
            FATALINFResponse response = (FATALINFResponse) Marshaller.unmarshal(element);
    ...
    public String getIdInquery() {
        return InquiryVersionEnum.getInquiryId(InquiryVersionEnum.ZAGS_FATAL).toString();
    }
}
