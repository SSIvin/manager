...
import smev.ElementServices.ZagsRojd.ROGDINFRequest;
import smev.ElementServices.ZagsRojd.ROGDINFResponse;
import smev.ElementServices.ZagsRojd.ФИОПрТип;
...
@Component
public class ROGDINFRequestRequest extends ObjectMessage implements TypeBussines, CreateMessageStrategy, ReceiveMessageStrategy {
    ...
    public ObjectMessage runOrder(BusinessMessageEntity messageEntity, UUID inquiryVersionId, PayLoad payLoad) throws ParseException, DatatypeConfigurationException {
        ...
        ObjectMessage message = new ROGDINFRequestRequest();
        ROGDINFRequest dataRequest = new ROGDINFRequest();
        SubjectDataDTO subjectDTO = (SubjectDataDTO) payLoad.getPayLoadObject();

        dataRequest.setКолДок(BigInteger.valueOf(1));
        dataRequest.setТипАГС("01");

        ROGDINFRequest.СведЗапрос.СведФЛ сведФЛ = new ROGDINFRequest.СведЗапрос.СведФЛ();
...
//        органЗАГСНТип.setНаимЗАГС();
        ...
        ROGDINFRequest.СведЗапрос сведЗапрос = new ROGDINFRequest.СведЗапрос();
...
//        сведЗапрос.setСведАГС(сведАГС);
        ...
        ROGDINFRequest.СведОсн сведОсн = new ROGDINFRequest.СведОсн();
        ...
        ((ROGDINFRequestRequest) message).data = dataRequest;
    ...
    public DataSubjectEntity runReceiveOrder(BusinessInteractionEntity interactionEntity, OrderInquiryDto orderInquiryDTO) throws ParseException, DatatypeConfigurationException {
        ...
        if (interactionInfoDto.getResponseNumber() > 0) {
            ...
            ROGDINFResponse response = (ROGDINFResponse) Marshaller.unmarshal(element);
    ...
    public String getIdInquery() {
        return InquiryVersionEnum.getInquiryId(InquiryVersionEnum.ZAGS_ROJD).toString();
    }
}
